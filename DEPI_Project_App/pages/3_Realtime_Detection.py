...
<style>
    ...
    * { scrollbar-width: thin; scrollbar-color: #00CCFF #020c1a; }
    ...
    .video-frame.active { animation: pulse-border 2s infinite alternate; }
    @keyframes pulse-border { 0% { border-color: rgba(0, 204, 255, 0.3); box-shadow: 0 0 10px 2px rgba(0, 204, 255, 0.2); } 100% { border-color: #00CCFF; box-shadow: 0 0 20px 3px rgba(0, 204, 255, 0.6); } }
        ...
        if not cap.isOpened():
            ...
            while st.session_state.run_rt:
                ...
                video_container.markdown(
                    f'<div class="video-frame active"><img src="data:image/jpeg;base64,{image_to_base64(final_rgb)}" width="100%"></div>',
                    unsafe_allow_html=True)
                ...
                frame_count += 1
                st.session_state.temp_session_data = {"id": len(st.session_state.history) + 1,
                                                      "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                                      "duration": total_elapsed, "frames_count": frame_count,
                                                      "snapshots": list(st.session_state.rt_snapshots),
                                                      "df": pd.DataFrame(list(st.session_state.rt_logs))}
                ...
                with log_placeholder.container():
                    if st.session_state.rt_logs:
                        df_log = pd.DataFrame(st.session_state.rt_logs)
                        df_display = df_log.iloc[::-1].head(20)[["Timestamp", "Class", "Confidence"]].copy()
    ...
    if not st.session_state.history:
        ...
        for session in reversed(st.session_state.history):
            with st.container():
                ...
                with c_info:
                    st.markdown(
                        f"""<div class="history-card"><div class="history-header"><span>SESSION LOG #{session['id']:02d}</span><span style="color:#FFF;">{session['duration']:.1f}s</span></div><div class="hist-meta"><i class="bi bi-calendar"></i> {session['timestamp']} &nbsp;|&nbsp; <i class="bi bi-images"></i> {len(session['snapshots'])} Snaps &nbsp;|&nbsp;<i class="bi bi-box"></i> {len(session['df'])} Detections</div></div>""",
                        unsafe_allow_html=True)
                with c_action:
                    st.markdown("<br>", unsafe_allow_html=True)
